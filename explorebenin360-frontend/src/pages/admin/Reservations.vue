<template>
  <div class="container-px mx-auto py-8">
    <h1 class="text-2xl font-bold mb-4">{{ t('admin.reservations') }}</h1>
    <div v-if="loading" class="text-sm text-[color:var(--color-text-muted)]">{{ t('common.loading') }}</div>
    <div v-else>
      <div v-if="items.length === 0" class="text-sm text-[color:var(--color-text-muted)]">{{ t('admin.no_reservations') }}</div>
