...
export type FavType = 'destination' | 'hebergement' | 'article' | 'guide'
...
export const useFavoritesStore = defineStore('favorites', {
  state: () => ({
    items: emptySets(),
    entities: reactive({ destination: {} as Record<number, any>, hebergement: {}, article: {}, guide: {} }) as Record<FavType, Record<number, any>>,
    usingServer: true,
    loaded: false,
  }),
  actions: {
    ...
    load() {
      try {
        ...
        if (raw) {
          ...
          ;(['destination','hebergement','article','guide'] as FavType[]).forEach((t) => {
            const arr = parsed[t] || []
            next[t] = new Set(arr)
          })
          this.items = next
    ...
    async syncOnLogin() {
      ...
      try {
        ...
        const server: Record<FavType, number[]> = res?.data || { destination: [], hebergement: [], article: [], guide: [] }
        ...
        for (const t of ['destination','hebergement','article','guide'] as FavType[]) {
